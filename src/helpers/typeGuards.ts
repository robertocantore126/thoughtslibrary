import type { BookResult, CustomResult, GameResult, MovieResult, MusicResult, Result, SearchTypes, ThoughtResult, TVResult } from '../types'
...
export function isThoughtResult(item: Result): item is ThoughtResult {
  if ((item as ThoughtResult).type === 'thought') {
    return true
  }
  else {
    return false
